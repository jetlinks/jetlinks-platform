...
importMiniui(function () {
    ...
    require(["request", "message", "miniui-tools"], function (request, message, tools) {
            ...
            grid.getColumn('action').renderer = function (e) {
                ...
                var html = [
                    tools.createActionButton("保存", "icon-save", function () {
                        ...
                        request.patch("autz-setting/", data, function (resp) {
                            console.log(data)
                            grid.unmask();
                            if (resp.status === 200) {
        ...
        var supports = {
            ...
            role: function () {
                ...
                $(".bind-role").on('click', function () {
                    tools.openWindow("admin/dimension/list.html?selector=1", "选中维度", "800", "600", function (data) {
                        if (data !== 'cancel' && data !== 'close') {
                            var data = {
                                type: 'role',
                                settingFor: data.id,
        ...
        function loadDimensionType() {
            request.get("dimension-type/all", function (resp) {
                if (resp.status === 200) {

                    //call(resp.result);
                } else {
                    message.showTips("加载维度类型失败:" + resp.message, "danger");
                }
            });
        }

        function loadAutzSettings(call) {
            request.get("autz-setting/_query" , request.encodeQueryParam({permissionId: permissionId}), function (resp) {
                if (resp.status === 200) {
