...
//引入miniui,miniui不支持require,使用此方式引入
    importMiniui(function () {
        ...
        function createReport(tab, id) {
            var el = tabs.getTabBodyEl(tab);
            require(['./report/report'], function (report) {
                report.createReport(id,function (report) {
                    report.render($(el))
                });
            });
        }
...
function initDashBoard() {
    ...
    require(['./report/report', 'request', "authorize"], function (report, request, authorize) {

        if (authorize.user.type === "open-api") {
            var html = "<iframe align=\"center\" width=\"100%\" height=\"100%\" src=\"../admin/data-standards/api/info.html\" frameborder=\"no\" border=\"0\" marginwidth=\"0\" marginheight=\"0\" scrolling=\"no\"></iframe>"
            $(".dashboard").append(html);
        } else {
            require(["css!admin/report/config/config.css"], function () {
                report.createReport("index",function (report) {
